...
- (void)setProgressValue:(CGFloat)progressValue {
    self.progressToAnimateTo = progressValue;
...
- (void)incrementAnimatingProgress {
    if (_progressValue >= self.progressToAnimateTo-0.01 && _progressValue <= self.progressToAnimateTo+0.01) {
        _progressValue = self.progressToAnimateTo;
        [self.animationTimer invalidate];
        [self setNeedsDisplay];
    } else {
        _progressValue = (_progressValue < self.progressToAnimateTo) ? _progressValue + 0.01 : _progressValue - 0.01;
        [self setNeedsDisplay];
    }
}
...
- (void)drawProgress:(CGContextRef)context withFrame:(CGRect)frame {
    CGRect rectToDrawIn = CGRectMake(0, 0, frame.size.width * self.progressValue, frame.size.height);
    CGRect insetRect = CGRectInset(rectToDrawIn, self.progressValue > 0.03 ? 0.5 : -0.5, 0.5);
...
//        NSArray *colors = @[(__bridge id)self.color.CGColor,(__bridge id)[UIColor whiteColor].CGColor];
