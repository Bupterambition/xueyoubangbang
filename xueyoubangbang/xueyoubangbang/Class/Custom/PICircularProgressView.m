...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat progressAngle = _progressValue * 360.0 - 90;
    ...
    {
        NSString *progressString = [NSString stringWithFormat:@"%.0f", _progressValue * 100.0];
...
{
    _progressValue = MIN(1.0, MAX(0.0, progressValue));
...
- (void)setProgressValue:(CGFloat)progressValue animated:(BOOL)animated
{
    if(animated){
        if(!timer){
            timer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:self selector:@selector(refreshProgress) userInfo:[NSNumber numberWithFloat:progressValue] repeats:YES];
        }
        else{
            [timer invalidate];
            timer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:self selector:@selector(refreshProgress) userInfo:[NSNumber numberWithFloat:progressValue]  repeats:YES];
...
{
    CGFloat finalValue = [((NSNumber *)timer.userInfo) doubleValue];
    if( (int) (finalValue * 100) == (int) (self.progressValue * 100) ){
        [timer invalidate];
        timer = nil;
    }
    else if((int) (finalValue * 100) < (int) (self.progressValue * 100)){
    ...
    {
        [self.delegate onRefresh:self.progressValue];
    }
}
