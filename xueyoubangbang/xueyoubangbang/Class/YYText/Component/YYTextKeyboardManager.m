...
- (CGRect)convertRect:(CGRect)rect toView:(UIView *)view {
    ...
    UIWindow *mainWindows = app.keyWindow;
    if (!mainWindows) mainWindows = app.windows.firstObject;
    if (!mainWindows) { // no window ?!
        if (view) {
    ...
    rect = [mainWindows convertRect:rect fromWindow:nil];
    if (!view) return [mainWindows convertRect:rect toWindow:nil];
    if (view == mainWindows) return rect;
    
    UIWindow *toWindow = [view isKindOfClass:[UIWindow class]] ? (id)view : view.window;
    if (!mainWindows || !toWindow) return [mainWindows convertRect:rect toView:view];
    if (mainWindows == toWindow) return [mainWindows convertRect:rect toView:view];
    
    // in different window
    rect = [mainWindows convertRect:rect toView:mainWindows];
    rect = [toWindow convertRect:rect fromWindow:mainWindows];
