...
@interface UIHomeworkAddItemController ()<UIGestureRecognizerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,QBImagePickerControllerDelegate>
{
    UIView *operation;
...
@implementation UIHomeworkAddItemController
...
- (void)createOperation
{
    ...
    if(_homeworkItem.picturesUIImage.count > 0)
    {
        UIButton *btnAdd = (UIButton*)[image0 viewWithTag:btnAddTag];
        btnAdd.hidden = YES;
        
        UIButton *btnPicture = (UIButton *)[image0 viewWithTag:btnPictureTag];
        [btnPicture setBackgroundImage:[_homeworkItem.picturesUIImage objectAtIndex:0] forState:UIControlStateNormal];
...
//    [btnPicture addTarget:self action:@selector(doDeletePicture:) forControlEvents:UIControlEventTouchUpInside];
    ...
    btnDeleteIcon.frame = CGRectMake(btnPicture.frame.size.width / 2 , - btnPicture.frame.size.height / 6, btnPicture.frame.size.width / 3 * 2, btnPicture.frame.size.height / 2);
...
//    _homeworkItem.audioData = recorder.amrAudio;
    [self.view addSubview:recorder];
    
    if(_homeworkItem.audioData != nil)
    {
        recorder.amrAudio = _homeworkItem.audioData;
        recorder.playerButton.hidden = NO;
        recorder.deleteButton.hidden = NO;
        recorder.recordButton.hidden = YES;
    }
}
...
- (NSArray *)getImageData
{
    ...
    for (UIView *sub in operation.subviews) {
        ...
        if(picture.hidden == NO)
        {
            ...
            NSString *filename;
            NSString *name = [NSString stringWithFormat:@"item_img_%d",item_imgscnt + 1];
...
- (void)doSure
{
    _homeworkItem.title = _homeworkItem.title?_homeworkItem.title:@"";
    _homeworkItem.desc = input.text;
    _homeworkItem.audioData = recorder.amrAudio;
    ...
    _homeworkItem.picturesUIImage = pictures;
    if(pictures.count > 0)
    {
        _homeworkItem.firstUIImage = [pictures objectAtIndex:0];
    }
    else
    {
        _homeworkItem.firstUIImage = nil;
    }
...
- (void)imagePickerController:(QBImagePickerController *)imagePickerController didSelectAssets:(NSArray *)assets
{
    ...
    for (int i = 0; i<assets.count; i++) {
        ...
        [btnPicture setBackgroundImage:[UIImage imageWithData:imageData] forState:UIControlStateNormal];
        btnPicture.hidden = NO;
