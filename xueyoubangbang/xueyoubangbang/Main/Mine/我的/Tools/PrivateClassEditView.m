...
#import "MClass.h"
#import "MGrade.h"
@interface PrivateClassEditView()
{
...
@end
@implementation PrivateClassEditView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(table == tableView)
    {
        ...
        else if(indexPath.row == 1)
        {
            ...
            {
                _label2.text = ((MGrade *)(gradeArray[indexPath.row])).name;
//                _label2.text = [gradeDic objectForKey:_currentGrade];
        ...
        else if(indexPath.row == 2)
        {
            ...
            {
                _label3.text = ((MClass *)(classArray[indexPath.row])).class_name;
    ...
    else
    {
        ...
        switch (tableResultType) {
            ...
            case 1:
                ...
                _currentGrade = ((MGrade *)(gradeArray[indexPath.row])).grade;
//                _currentGrade = [self gradeIdByGradeName:_label2.text];
            ...
            case 2:
                ...
                _currentClass = ((MClass *)(classArray[indexPath.row])).class;
...
- (NSString *)tableResultDataText:(NSInteger)row
{
    switch (tableResultType) {
        case 0:
            ...
            break;
        case 1:
            return ((MGrade *)(gradeArray[row])).name;;
            break;
        case 2:
            return ((MClass *)(classArray[row])).class_name;
//            return [classDic objectForKey:[NSString stringWithFormat:@"%ld",row]];
            break;
        default:
            return nil;
    }
...
- (void)searchSchool:(NSString *)text
{
    ...
    [AFNetClient GlobalGet:kUrlgetPrivateSchool parameters:@{@"input":text} success:^(AFHTTPRequestOperation *operation, NSDictionary *dataDict) {
...
{
    //这部分是我加上去的
//    [gradeDic removeAllObjects];//清空
//    [gradeIdArray removeAllObjects];
    [AFNetClient GlobalGet:kUrlgetGradeList parameters:@{@"school_id":_currentSchoolId} success:^(AFHTTPRequestOperation *operation, NSDictionary *dataDict)
     {
         ...
         if(isUrlSuccess(dataDict))
         {
             ...
             if(![dataDict[@"list"] isKindOfClass:[NSString class]])
             {
                 NSArray *array = [dataDict objectForKey:@"list"];;
                 for (NSUInteger i=0; i<array.count; i++)
                 {
                     [gradeArray addObject:[MGrade objectWithDictionary:array[i]]];
                 }
             }
...
-(void)getGradeClass
{
    [AFNetClient GlobalGet:kUrlgetGradeClass parameters:@{@"school":_currentSchoolId,@"grade":_currentGrade} success:^(AFHTTPRequestOperation *operation, NSDictionary *dataDict)
     {
         ...
         [MBProgressHUD hideAllHUDsForView:mainWindow animated:YES];
          if(isUrlSuccess(dataDict))
          {
              if(![dataDict[@"list"] isKindOfClass:[NSString class]] )
              {
                  ...
                  for (NSUInteger i=0; i<array.count; i++)
                  {
                      [classArray addObject:[MClass objectWithDictionary:array[i]]];
                  }
              }
